...
export default class ReservationDetailsM extends Component {
  setData() {
    const statusTxt = {
  ...
  created() {
    this.popup = new Popup({ target: this.$target })
    
    this.eSubscribe('selectReservation', (reserv) => {
      if (deepEqual(this.props.reservation, reserv) === false) {
        this.props.reservation = reserv
      }
      this.popup.setContents(this.template())
      this.popup.toggle()
    })
  }
